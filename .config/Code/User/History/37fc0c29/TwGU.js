const readline = require('node:readline').createInterface({
  ...
  const operations = ['addition', 'subtraction', 'multiplication', 'division', 'exponent'];
    ...
    if (numbers.length < 2) {
      ...
      if (operationIndex < operations.length) {
        getNumber(`Enter the next number for the next operation: `, (num) => {
          // For subsequent operations, the result of the previous is implicitly used with the new number
  ...
  function performIntermediateCalculation(num1, num2, operation) {
    switch (operation) {
      ...
      case 'multiplication':
        return num1 * num2;
      case 'division':
        return num2 !== 0 ? num1 / num2 : NaN; // Handle division by zero
      case 'exponent':
        return num1 ** num2;
      default:
        return NaN;
  ...
  console.log('Sequential Math Operations Program (9 Inputs)');
  processInput();